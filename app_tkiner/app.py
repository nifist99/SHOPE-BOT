...
def api_info():
        api_url='https://tanlalana.com/api/info'
        api_data={'nama':"shope"}
        r = requests.get(api_url, data=api_data)
        global info_ap
        info_ap = r.json()
...
def info():
        ...
        text_area.insert(END,info_ap['data']['keterangan'])
        # Making the text read only
        text_area.configure(bg="black",fg="green")
        win.mainloop() 
...
class boot:
        ...
        def aturKomponen(self):
                ...
                self.btnInfo = Button(btnFrame, text='Info',\
                                        command=info, width=10,\
                                        relief=FLAT, bd=2, bg="#3DB2FF", fg="white",activebackground="#444",activeforeground="white" )
                self.btnInfo.grid(row=0, column=4,padx=20,pady=10,sticky=W)
